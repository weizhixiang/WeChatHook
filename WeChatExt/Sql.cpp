...
void DatabaseHwnd(int dbAddress, int dbHandle)
{
	...
	//MessageBoxA(NULL, Db.path, "错误", MB_OK);
...
int RunSql(string dbName, string sqlStr, sqlite3_callback callBack, char*& sqlErrmsg)
{
	...
	for (auto& db : GetDbHandleList())
	{
		if (StrStrA(db.path, dbName.c_str()))
		{
			...
			char hexString[12] = { 0 };
			sprintf_s(hexString, "0x%08X", db.handler);
			//MessageBoxA(NULL, hexString, "错误", MB_OK);
			return 0;
		}
	}
	return 1;
...
INT __cdecl RunSqlCallBack(void* para, int nColumn, char** colValue, char** colName)
{
