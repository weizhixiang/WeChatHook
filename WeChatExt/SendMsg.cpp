...
/**
 * 发送文本消息
 * @param wchar_t* wxid, wchar_t* msg
 * @return void
 */
void SendTextMsg(wchar_t* wxid, wchar_t* msg)
{
	...
	//MessageBox(NULL, wxid, L"sendMsg", MB_OK);
	...
	//wchar_t sender[0x300] = { 0 };
	//swprintf_s(sender, L"发送消息:%s,%s", wxid, msg);
	//Send(Cmd_Init, result, "", 0, sender);
	__asm {
		push 0x1
		push 0x0
		mov edx, pWxid
