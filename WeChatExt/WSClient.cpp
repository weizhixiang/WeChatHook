...
void InitWebSocket() {
	...
	//MessageBoxA(NULL, strPosixTime.c_str(), "错误", MB_OK);
Start:
	try
	{
	...
	Next:
		...
		Run:
			try
			{
				switch (con->get_state()) // enum value {connecting = 0,open = 1,closing = 2,closed = 3};
				{
				case 0:
					Sleep(1000);//收消息不延迟
					break;
...
void RecvMessage(client* wsclient, websocketpp::connection_hdl hdl, message_ptr msg) {
	...
	try
	{
		...
		case Cmd_SendTextMessage:
			...
			//MessageBoxA(NULL, WxId.c_str(), "sendMsg", MB_OK);
			break;
...
void Send(int code, neb::CJsonObject data, string callback, int err, wstring msg) {
	try
	{
		...
		json.Add("Pid", (int)GetCurrentProcessId());//遗留问题
